...
-- LINK 2 https://habr.com/ru/post/63883/
...
CREATE PROCEDURE create_node(
		IN n_parent_id INT(10),
		IN n_name VARCHAR(155),
		IN n_tree INT(10),
		IN n_left_key INT(10)
)
BEGIN
	SET @left_key := 0;
    SET @level := 0;
...
CREATE PROCEDURE update_node(IN n_id INT(10), IN new_parent_id INT(10), IN n_left_key INT(10))
BEGIN
...
-- Если не parent_id, то изменен left_key, или если изменение parent_id ничего не дало
      IF @left_key IS NULL OR @left_key = 0 THEN
          SELECT id, left_key, right_key, `level`, parent_id
              INTO @tmp_id, @tmp_left_key, @tmp_right_key, @tmp_level, @tmp_parent_id
              FROM ns_tree
              WHERE tree = @new_tree AND (right_key = @new_left_key OR right_key = @new_left_key - 1) AND parent_id = new_parent_id
              LIMIT 1;
